    ...
    while (pos < input_length)
    {
        ...
        case '/':
            if (pos + 1 <= input_length && input[pos + 1] == '/')
            {
                pos += 2;
                look_ahead_pos = pos;
                while (look_ahead_pos <= input_length && is_space(input[look_ahead_pos]))
                {
                    look_ahead_pos++;
                }
                ...
                tokens.push_back({TokenType::SingleLineComment,
                                  line_number,
                                  pos - col_acc,
            ...
            if (pos + 1 <= input_length && input[pos + 1] == '*')
            {
                ...
                tokens.push_back({TokenType::MultiLineComment,
                                  line_number,
                                  pos - col_acc,
                                  std::string_view(&input[pos], look_ahead_pos - pos)});
        ...
        case 'Z':
            ...
            tokens.push_back({is_keyword(std::string_view(&input[pos], look_ahead_pos - pos)) ? TokenType::Keyword : is_dataType(std::string_view(&input[pos], look_ahead_pos - pos)) ? TokenType::DataType
            ...
            break;
        case '0':
            if (input_length >= pos + 2 && input[pos + 1] == 'x' || input[pos + 1] == 'X')
            { // is Hex
                if (!is_decimal(input[pos + 2]))
                { // Empty Hex 0x[NonDigit] return as Unknown
                    tokens.push_back({TokenType::Unknown,
                                      line_number,
                                      pos - col_acc,
                                      std::string_view(&input[pos], look_ahead_pos - pos)});
                ...
                tokens.push_back({TokenType::Hexadecimal,
                                  line_number,
                                  pos - col_acc,
                                  std::string_view(&input[pos], look_ahead_pos - pos)});
                pos = look_ahead_pos++;
